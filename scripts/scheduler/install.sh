...
if [[ "$MODE" == "status" ]]; then
    ...
    systemctl list-timers "ascendo-ubuntu*" 2>/dev/null || true
    exit 0
fi
...
if [[ "$MODE" == "remove" ]]; then
    ...
    sudo rm -f /etc/systemd/system/ascendo-ubuntu@.service \
               /etc/systemd/system/ascendo-ubuntu@.timer
...
[Unit]
Description=Ascendo Ubuntu — Scheduled update (${PROFILE})
...
[Unit]
Description=Ascendo Ubuntu — Scheduled update timer
...
systemctl list-timers "ascendo-ubuntu*" || true
