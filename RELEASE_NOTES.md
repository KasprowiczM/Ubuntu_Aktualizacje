...
- `bin/ascendo-ubuntu` subcommands: `settings`, `health`, `exclusions`.
...
- `bin/ascendo-ubuntu` CLI shim with auto-resolved root (`ASCENDO_ROOT` > `/opt/ascendo-ubuntu` > repo).
- User Journey docs in EN + PL (`docs/{en,pl}/user-journey.md`) covering 6 flows.

### Changed
- `.deb` metadata: `Package: ascendo`, `Source: ascendo-ubuntu`, version 0.3.0.
...
### Added
- `.deb` packaging: `packaging/build-deb.sh`, control/postinst/prerm, `/usr/bin/ascendo-ubuntu-ubuntu` shim with `fresh|dashboard|schedule|snapshot` subcommands.
- First-run wizard modal triggered when `~/.config/ascendo-ubuntu/onboarded.json` missing; `GET /onboarding/state`, `POST /onboarding/complete`.
- Bearer token middleware (`app/backend/auth.py`) + `/auth/{status,generate-token,revoke-token}`; no-op when token file absent.
- Audit log (`app/backend/audit.py`) JSONL at `~/.local/state/ascendo-ubuntu/audit.log`; `/audit` endpoint tail.
...
### Changed
- `update-all.sh` reads sudo password ONCE; ephemeral 0700 askpass helper at `$XDG_RUNTIME_DIR/ascendo-ubuntu/`; `lib/common.sh` wraps `sudo` as `sudo -A`.
...
- systemd timer (`ascendo-ubuntu.{service,timer}` + `install-timer.sh`) replacing cron.
