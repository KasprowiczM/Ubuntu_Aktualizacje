...
DEB=$(find "${TAURI_DIR}/target" -iname '*ascendo-ubuntu-desktop*.deb' 2>/dev/null | head -1)
APPIMG=$(find "${TAURI_DIR}/target" -iname '*ascendo-ubuntu-desktop*.AppImage' 2>/dev/null | head -1)
...
if [[ -n "$DEB" ]]; then
    ...
    echo "  ascendo-ubuntu-desktop      # Standalone desktop binary on PATH"
    echo "  # or via Activities menu (look for 'ascendo-ubuntu-desktop')"
    echo
