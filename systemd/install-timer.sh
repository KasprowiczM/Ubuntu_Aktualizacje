...
SERVICE_NAME="ascendo-ubuntu@${CURRENT_USER}"
TIMER_NAME="ascendo-ubuntu@${CURRENT_USER}"
...
echo -e "\n${BOLD}${BLUE}── Ascendo Ubuntu systemd timer ──${RESET}\n"
...
if [[ "$MODE" == "status" ]]; then
    ...
    echo ""
    echo "── Next runs:"
    systemctl list-timers "ascendo-ubuntu*" 2>/dev/null || echo "  (no timers)"
    exit 0
fi
...
if [[ "$MODE" == "remove" ]]; then
    ...
    sudo rm -f /etc/systemd/system/ascendo-ubuntu@.service
    sudo rm -f /etc/systemd/system/ascendo-ubuntu@.timer
    sudo rm -f /etc/systemd/system/ascendo-ubuntu.timer
...
sudo tee /etc/systemd/system/ascendo-ubuntu@.service > /dev/null << EOF
[Unit]
Description=Ascendo Ubuntu — Full System Update
...
sudo tee /etc/systemd/system/ascendo-ubuntu@.timer > /dev/null << 'EOF'
[Unit]
Description=Ascendo Ubuntu — Weekly update timer

[Timer]
OnCalendar=Sun *-*-* 03:00:00
Persistent=true
RandomizedDelaySec=30min
Unit=ascendo-ubuntu@%i.service
...
systemctl list-timers "ascendo-ubuntu*" 2>/dev/null | grep -E "(NEXT|ascendo)" || true
echo ""
echo "── Commands:"
